...
//	Número de Ordem
	...
	public void Write ( Writer w ) throws IOException 
	{
		String [] args = new String [] {
			...
			Math.round(Optional.ofNullable(valor_base_incidencia).orElse(0d)) + "",
			Math.round(Optional.ofNullable(taxa_iva).orElse(0d)) + "" ,
			Math.round(Optional.ofNullable(iva_suportado).orElse(0d)) + "",
			Math.round(Optional.ofNullable(direito_dedutivel).orElse(0d)) + "",
